...
async def campaign_page(request: Request):
    ...
    return templates.TemplateResponse("campaign.html", {"request": request, "engine_state": engine_state, "config": config, "stats": stats})

@router.get("/campaign/api/state")
async def get_engine_state_api():
    return {"engine_state": await database.get_engine_state(), "config": await database.get_config(), "stats": await database.get_stats()}
